...
time_steps = 12
outer_n_splits = 8
inner_n_splits = outer_n_splits_options[outer_n_splits][1]  # 예시로 첫 번째 값을 선택
learning_rate = 0.001
...
# 이상치 처리 함수
def handle_outliers_zscore(data, columns, threshold=2):
    for column in columns:
        if column == '누적광량(J/cm²)':
            continue
...
handle_outliers_zscore(data, features)
...
with open('temp_model_performance.txt', 'w') as f:
    ...
    f.write(f"scaler X : std\n")
    f.write(f"scaler Y : std\n")
